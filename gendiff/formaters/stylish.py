...
def stringify_stylish(value, replacer=' ', spaces_count=4):
    ...
    def inner(current_value, depth):
        ...
        deep_ind_size = depth + spaces_count
        deep_indent = replacer * deep_ind_size
        current_indent = replacer * depth
        lines = []
        for key, val in current_value.items():
            if key[0] == '+' or key[0] == '-':
                sign_indent = replacer * (deep_ind_size - 2)
                lines.append(f'{sign_indent}{key}: {inner(val, deep_ind_size)}')
            else:
                lines.append(f'{deep_indent}{key}: {inner(val, deep_ind_size)}')
